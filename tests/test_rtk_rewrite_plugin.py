...
# Upstream layout (hooks/hermes/rtk-rewrite) vs flat repo layout (plugin at repo root)
_PLUGIN_CANDIDATES = [
    Path(__file__).resolve().parents[1] / "rtk-rewrite" / "__init__.py",
    Path(__file__).resolve().parents[1] / "__init__.py",
]
PLUGIN_PATH = next((p for p in _PLUGIN_CANDIDATES if p.exists()), _PLUGIN_CANDIDATES[0])
